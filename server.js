...
const server=http.createServer(async(req,res)=>{
  ...
  if(req.method==='GET'&&(url.pathname==='/.well-known/agent-capabilities.json'||url.pathname==='/capabilities')){
    ...
    return json(res,200,{name:'Voice AI Margin Intelligence',version:'0.4.2',description:'Structured AI voice pricing and margin benchmarks for agents.',capabilities:[{id:'voice-margin-index',description:'Compare AI voice retail pricing, revenue per minute, and gross-margin ceilings.',method:'GET',url:`${publicUrl}/api/voice-margin-index`,price:'free'},{id:'mcp',description:'Remote MCP server for voice AI pricing and margin intelligence',method:'POST',url:`${publicUrl}/mcp`,price:'free'}],mcp:`${publicUrl}/mcp`,discovery:`${publicUrl}/.well-known/agent-capabilities.json`,privacy:`${publicUrl}/privacy`,support:`${publicUrl}/support`,usage:`${publicUrl}/usage`},'public, max-age=60');
  }
  ...
  if(req.method==='POST'&&url.pathname==='/mcp'){
    ...
    try{
      ...
      if(method==='initialize') return json(res,200,mcpResult(id,{protocolVersion:'2025-06-18',capabilities:{tools:{}},serverInfo:{name:'voice-ai-margin-intelligence',version:'0.4.2'},instructions:'Use voice_margin_index when comparing AI voice providers, retail pricing, revenue per minute, or gross-margin economics. This server is read-only.'}));
